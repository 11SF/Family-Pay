...
export default function CardYoutube({ name,pic,lastDate,expireDate }) {
    ...
    const status_func = (status)=> {
        switch(status) {
            case "1" : return {"class" : "card_body_green", "status" : "ดูฟรีม่ายยมีโฆษณา"};
            case "2" : return {"class" : "card_body_yellow", "status" : "ถึงเวลาจ่ายแล้วครับผมม"};
            case "3" : return {"class" : "card_body_red", "status" : "Ad. is coming in 3..2..1.."};
            default :
                return "err";
        }
    }
    return (
        <div className="card text-left" data-aos="zoom-in">
